...
import { Branch, MinistryAudit } from '@/api/entities';
...
import { Plus, FileText, Eye, Calendar, User, AlertCircle, RefreshCw, Building, UserCheck, Briefcase, Archive, PlusCircle } from 'lucide-react'; // Added PlusCircle
import { format } from 'date-fns';
import { he } from 'date-fns/locale';
import ExportButton from '../components/ExportButton';
import MinistryInternalAuditForm from '../components/MinistryInternalAuditForm';

export default function MinistryAudits() {
    ...
    const [isInternalAuditFormOpen, setIsInternalAuditFormOpen] = useState(false);
    ...
    const loadData = async () => {
        ...
        try {
            ...
            setMinistryAudits(auditsWithBranches);
            setBranches(branchesData);
        } catch (error) {
            console.error("Error loading ministry audits:", error);
    ...
    const handleSubmit = async (e) => {
        ...
        setIsSaving(true);
        try {
            await MinistryAudit.create({ ...formData, source: 'external_report' });
            alert('דוח התמ"ת החיצוני נשמר בהצלחה!');
            setIsFormOpen(false);
            setFormData({
                branch_id: '',
                audit_date: new Date().toISOString().split('T')[0],
    ...
    const handleViewDetails = (audit) => {
        window.location.href = createPageUrl(`MinistryAuditDetails?id=${audit.id}`);
    };
    
    const internalAudits = ministryAudits.filter(a => a.source === 'internal_audit');
    const externalAudits = ministryAudits.filter(a => a.source === 'external_report');
        ...
        <>
            <div className="space-y-6">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold">ביקורת משרד התמ"ת</h1>
                    <div className="flex gap-2">
                        <ExportButton audits={ministryAudits} reportName="דוח ביקורות משרד התמ'ת" />
                        <Button 
                            onClick={() => setIsInternalAuditFormOpen(true)}
                            className="bg-blue-600 hover:bg-blue-700 gap-2"
                        >
                            <PlusCircle className="w-4 h-4" />
                            ביקורת פנימית
                        </Button>
                        <Button onClick={() => setIsFormOpen(true)} className="bg-orange-600 hover:bg-orange-700 gap-2">
                            <PlusCircle className="w-4 h-4" />
            ...
            <MinistryInternalAuditForm
                open={isInternalAuditFormOpen}
                onOpenChange={setIsInternalAuditFormOpen}
                onSave={loadData}
            />

            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen} dir="rtl">
                <DialogContent className="sm:max-w-[600px]">
                    <DialogHeader><DialogTitle>הוספת ביקורת משרד התמ"ת חיצונית</DialogTitle><DialogDescription>רישום ביקורת שהתקבלה ממשרד התמ"ת או גורם חיצוני אחר.</DialogDescription></DialogHeader>
                    <form onSubmit={handleSubmit} className="space-y-4 py-4">
                        ...
                        <DialogFooter>
                            ...
                            <Button type="submit" disabled={isSaving} className="bg-orange-600 hover:bg-orange-700">{isSaving ? 'שומר...' : 'שמור ביקורת'}</Button>
                        </DialogFooter>
                    </form>
                </DialogContent>
            </Dialog>
