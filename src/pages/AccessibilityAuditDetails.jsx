...
export default function AccessibilityAuditDetails() {
    ...
    const loadAudit = async (auditId) => {
        try {
            const auditData = await AccessibilityAudit.get(auditId);
            setAudit(auditData);
        } catch (error) {
    ...
    const responses = audit.accessibility_responses?.responses || [];

    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                <Link to={createPageUrl('AccessibilityAudits')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900">
                    <ArrowLeft className="w-4 h-4" />
                    חזרה לביקורות נגישות
                </Link>
                <h1 className="text-2xl font-bold flex items-center gap-2">
                    <Shield className="w-6 h-6 text-blue-600" />
                    פרטי ביקורת נגישות פנימית
                </h1>
            </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>שאלות ותשובות נגישות</CardTitle>
                </CardHeader>
                <CardContent>
                    {responses.length > 0 ? (
                        ...
                        <div className="text-center py-8 text-gray-500">
                            ...
                            <p>לא נמצאו תשובות רלוונטיות לנושא נגישות בביקורת זו</p>
                        </div>
                    )}
                </CardContent>
            </Card>
