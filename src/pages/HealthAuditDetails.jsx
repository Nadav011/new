...
import { ArrowLeft, Calendar, User, Building, FileText, Heart } from 'lucide-react';
import { format } from 'date-fns';

export default function HealthAuditDetails() {
    ...
    const loadAudit = async (auditId) => {
        try {
            const auditData = await HealthAudit.get(auditId);
            setAudit(auditData);
        } catch (error) {
    ...
    const responses = audit.health_responses?.responses || [];

    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                <Link to={createPageUrl('HealthAudits')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900">
                    <ArrowLeft className="w-4 h-4" />
                    חזרה לביקורות תברואה
                </Link>
                <h1 className="text-2xl font-bold flex items-center gap-2">
                    <Heart className="w-6 h-6 text-green-600" />
                    פרטי ביקורת תברואה פנימית
                </h1>
            </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>שאלות ותשובות תברואה</CardTitle>
                </CardHeader>
                <CardContent>
                    {responses.length > 0 ? (
                        ...
                        <div className="text-center py-8 text-gray-500">
                            ...
                            <p>לא נמצאו תשובות רלוונטיות לנושא תברואה בביקורת זו</p>
                        </div>
                    )}
                </CardContent>
            </Card>
